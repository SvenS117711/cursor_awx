        ...
        <div class="content-wrapper">
            <div class="jobs-panel">
                ...
                <div class="search-container" style="margin-bottom: 10px;">
                    <input type="text" id="jobSearchInput" placeholder="Jobs durchsuchen nach Name" onkeyup="updateJobs()">
                </div>
                <div id="jobsList">
    ...
    <script>
        ...
        function updateJobs() {
            ...
            fetch('/get_jobs')
                ...
                .then(jobs => {
                    // Lese den Suchbegriff aus dem Input-Feld aus
                    var searchValue = document.getElementById('jobSearchInput').value.toLowerCase();

                    // Filtere die Jobs nach dem Namen (nur wenn etwas eingegeben wurde)
                    if (searchValue) {
                        jobs = jobs.filter(job => job.name.toLowerCase().includes(searchValue));
                    }

                    const jobsList = document.getElementById('jobsList');
                    if (!jobsList) {
                        console.error("❌ Fehler: Element mit ID 'jobsList' nicht gefunden!");
                        return;
                    }
